...
impl DynamicHuffmanCodes {
	...
	fn from_lengths(lengths:&[u4]) -> DynamicHuffmanCodes {
		let mut lengths:Vec<DynamicHuffmanCodeValue> = lengths
			...
			.cloned()
			.zip(0..)
			.map(|(len, val)| DynamicHuffmanCodeValue {
				length:len,
				value:val,
			})
...
mod tests {
	...
	mod dynamic_huffman_codes {
		...
		fn two_constant_length() {
			...
			assert_eq!(
				0,
				dut.decode(&mut Bits::new([0b0u8].iter().cloned())).unwrap()
			);
			assert_eq!(
				1,
				dut.decode(&mut Bits::new([0b1u8].iter().cloned())).unwrap()
			);
		}
		#[test]
		fn four_constant_length() {
			...
			assert_eq!(
				0,
				dut.decode(&mut Bits::new([0b00u8].iter().cloned()))
					.unwrap()
			);
			assert_eq!(
				1,
				dut.decode(&mut Bits::new([0b10u8].iter().cloned()))
					.unwrap()
			);
			assert_eq!(
				2,
				dut.decode(&mut Bits::new([0b01u8].iter().cloned()))
					.unwrap()
			);
			assert_eq!(
				3,
				dut.decode(&mut Bits::new([0b11u8].iter().cloned()))
					.unwrap()
			);
		}
		#[test]
		fn four_constant_delta() {
			...
			assert_eq!(
				0,
				dut.decode(&mut Bits::new([0b000_0u8].iter().cloned()))
					.unwrap()
			);
			assert_eq!(
				1,
				dut.decode(&mut Bits::new([0b00_01u8].iter().cloned()))
					.unwrap()
			);
			assert_eq!(
				2,
				dut.decode(&mut Bits::new([0b0_011u8].iter().cloned()))
					.unwrap()
			);
			assert_eq!(
				3,
				dut.decode(&mut Bits::new([0b_0111u8].iter().cloned()))
					.unwrap()
			);
		}
		#[test]
		fn provided_sample() {
			...
			assert_eq!(
				5,
				dut.decode(&mut Bits::new([0b00_00u8].iter().cloned()))
					.unwrap()
			);
			assert_eq!(
				0,
				dut.decode(&mut Bits::new([0b0_010u8].iter().cloned()))
					.unwrap()
			);
			assert_eq!(
				1,
				dut.decode(&mut Bits::new([0b0_110u8].iter().cloned()))
					.unwrap()
			);
			assert_eq!(
				2,
				dut.decode(&mut Bits::new([0b0_001u8].iter().cloned()))
					.unwrap()
			);
			assert_eq!(
				3,
				dut.decode(&mut Bits::new([0b0_101u8].iter().cloned()))
					.unwrap()
			);
			assert_eq!(
				4,
			...
			assert_eq!(
				7,
				dut.decode(&mut Bits::new([0b_1111u8].iter().cloned()))
					.unwrap()
			);
		}
	}
	...
	mod inflate_samples {
		...
		fn abcde_times_five() {
			let exp:[u8; 25] = [
				0x61, 0x62, 0x63, 0x64, 0x65, 0x61, 0x62, 0x63, 0x64, 0x65, 0x61, 0x62, 0x63, 0x64,
				0x65, 0x61, 0x62, 0x63, 0x64, 0x65, 0x61, 0x62, 0x63, 0x64, 0x65,
			];
		...
		fn black_square_4x4() {
			...
			let dut:[u8; 21] = [
				0x9d, 0xc8, 0xb1, 0x0d, 0x00, 0x00, 0x00, 0x82, 0x30, 0xff, 0x7f, 0x5a, 0x1d, 0x99,
				0x21, 0x61, 0x69, 0x5e, 0xb9, 0x80, 0x01,
			];
			let mut dut = dut.iter().cloned();
			let res = inflate(&mut dut).unwrap();
